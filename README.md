...
当前已支持：文字单题/多题自动拆分、PNG/JPEG/WebP 题图的多题框检测与可编辑裁剪、按题目批量创建独立答疑 session、OpenAI Responses / OpenAI-compatible Chat Completions / Anthropic Messages 三协议加密模型配置、自动同步的 OpenCode 免费模型、可选选项或直接输入原文回应的检查点选择题、跨 session 的全局知识卡片/题目卡片库、按试卷稳定归档的层级卡片文件夹与复制/剪切/移动、知识/题目卡片固定双列 PDF 导出与只显示题目摘要的练习模式、SQLite 历史会话与删除、按 session 严格递增的 durable events 与断线重放 SSE，以及 JSONL/Markdown 双份诊断日志。页面采用左侧会话、中央对话、右侧卡片的三栏布局；建会话和会话内回复共用底部输入框，不再把“题目”和“你想到哪一步”拆成两个表单。
...
左侧会话栏直接从 SQLite 读取；“历史搜题”只作为不可跳转的树标题，下面仍保留可搜索、可展开的“试卷 → 题目”快速树，旧会话显示在“未分类题目”中，“清空全部会话”也只保留在这里。“知识卡片库”“错题卡片库”“错题集”是三个平级一级入口，不再使用“错题库”父级与二级菜单。“错题卡片库”以已归档 `problem_card` 为唯一数据源，按卡片所属的受管试卷目录分组；未生成或未保存题目卡片的 session 不会出现，删除会话或活动试卷也不会移除已经入库的卡片。点击题目条目直接打开只读题目卡片，不再请求或切回原答疑 session；只有用户主动删除卡片时它才从库中消失。三个库都支持先进入多选再经确认批量删除；知识/题目卡片删除不会删除来源会话，错题集删除只删除对应快照而保留题目卡片。跨卷多选和 `POST /api/mistake-sets` 传递有序 `card_ids`，因此来源会话已删除的卡片仍可保存或打印为独立 SQLite 快照。“错题集”展示所有已保存或打印的快照，点击后打开该错题集的 PDF/打印预览。

当前打开、正在运行或存在并发打开/删除请求的会话不可单条删除，中央不提供手工试卷级删除。删除某试卷的最后一个 session 时，后端在同一 SQLite 事务内同步删除该 `exam_papers` 实体，使它从会话快速树和新建题图的选卷列表中消失；对应受管卡片目录和已归档卡片继续保留在知识/错题卡片库中。“清空全部会话”也会清空全部活动试卷，但不会删除已归档知识卡和题目卡；尚未保存的临时卡片仍随来源 session 删除。“清空全部卡片”除检查浏览器活动流外，还会在同一 SQLite 写事务内复查权威 `session_runs`；只要存在 `queued/running` run 就整体返回 409 且不删除任何卡片，避免断流后的后台生成与清理交错。浏览器记住的活动 session 若已因清空数据库、切换 SQLite 文件或其他窗口删除而不存在，刷新时会自动清理该陈旧引用并回到新题界面，不显示 404；侧栏并发删除产生的失效条目也会自动移除。原有 `POST /api/sessions/restore` 仍保留给需要显式创建实验分支的调用方。
...
答疑页不再显示学习卡片栏按钮，也不再挂载右侧文件管理器。知识卡片库与错题卡片库都先在试卷/卡片列表进入多选、跨卷选择，再以 A4 纵向固定双列紧凑导出。知识卡片和题目卡片打印内容分别复用答疑卡片的完整字段；错题导出可开启“只看题目摘要”，用空白作答区替代解析与答案。导出会打开系统打印面板，选择“另存为 PDF”即可保留 KaTeX 公式与彩色版式。
