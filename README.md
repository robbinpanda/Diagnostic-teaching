...
当前已支持：文字单题/多题自动拆分、PNG/JPEG/WebP 题图的多题框检测与可编辑裁剪、按题目批量创建独立答疑 session、OpenAI Responses / OpenAI-compatible Chat Completions / Anthropic Messages 三协议加密模型配置、自动同步的 OpenCode 免费模型、可选选项或直接输入原文回应的检查点选择题、跨 session 的全局知识卡片/题目卡片库、按试卷稳定归档的层级卡片文件夹与复制/剪切/移动、知识卡片库内多选后的 PDF 多排版导出、SQLite 历史会话与删除、按 session 严格递增的 durable events 与断线重放 SSE，以及 JSONL/Markdown 双份诊断日志。页面采用左侧会话、中央对话、右侧卡片的三栏布局；建会话和会话内回复共用底部输入框，不再把“题目”和“你想到哪一步”拆成两个表单。
...
左侧会话栏直接从 SQLite 读取；“历史搜题”只作为不可跳转的树标题，下面仍保留可搜索、可展开的“试卷 → 题目”快速树，旧会话显示在“未分类题目”中，“清空全部会话”也只保留在这里。“知识卡片库”“错题卡片库”“错题集”是三个平级一级入口，不再使用“错题库”父级与二级菜单。“错题卡片库”复用按试卷分组的题目总览与单卷题目页，可跨试卷多选题目并进入 A4 打印预览；“仅保存”或“保存并打印”都会通过 `POST /api/mistake-sets` 把题目文字、题图和来源试卷保存为 SQLite 快照。“错题集”展示所有已保存或打印的快照，点击后打开该错题集的 PDF/打印预览，而不是重新罗列可跳转的 session。错题卡片库由真实 `SessionHistoryItem` 派生，在视口宽度 `> 1100px`、`761–1100px` 和 `≤ 760px` 时分别显示 3、2、1 列真实试卷卡片，可同时搜索试卷名和题目名，并按“最近更新”或“名称排序”。历史树与错题卡片库中的题目仍通过 `GET /api/sessions/{session_id}` 打开原 session，不会因浏览而复制记录。

当前打开、正在运行或存在并发打开/删除请求的会话不可单条删除，中央不提供手工试卷级删除。删除某试卷的最后一个 session 时，后端在同一 SQLite 事务内同步删除该 `exam_papers` 实体，使它从错题卡片库、快速树和新建题图的选卷列表中消失；“清空全部会话”也会清空全部试卷。已归档知识卡和题目卡不会随 session 或试卷删除，尚未保存的临时卡片则随来源 session 删除。“清空全部卡片”除检查浏览器活动流外，还会在同一 SQLite 写事务内复查权威 `session_runs`；只要存在 `queued/running` run 就整体返回 409 且不删除任何卡片，避免断流后的后台生成与清理交错。浏览器记住的活动 session 若已因清空数据库、切换 SQLite 文件或其他窗口删除而不存在，刷新时会自动清理该陈旧引用并回到新题界面，不显示 404；侧栏并发删除产生的失效条目也会自动移除。原有 `POST /api/sessions/restore` 仍保留给需要显式创建实验分支的调用方。
...
答疑页不再显示学习卡片栏按钮，也不再挂载右侧文件管理器。知识卡片导出迁入中央“知识卡片库”，与“错题卡片库”一致先在试卷/卡片列表进入多选、跨卷选择，再打开只负责确认单/双/三列排版的 PDF 弹窗；仅已归档 `knowledge_card` 可选。“错题卡片库”继续使用现有跨试卷多选与错题集打印样式。导出会打开系统打印面板，选择“另存为 PDF”即可保留 KaTeX 公式与彩色版式。
