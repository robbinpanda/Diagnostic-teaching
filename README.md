...
普通用户可从 [GitHub Releases](https://github.com/robbinpanda/Diagnostic-teaching/releases/latest) 下载 `Diagnostic-Teaching-Setup-0.5.0-x64.exe`。安装版包含前后端、Python 运行时和语音依赖，不要求另装 Node.js、Python、Conda 或 SQLite。

> 从 0.4.0 第一次升级到 0.5.0 会清理 `%APPDATA%\DiagnosticTeaching` 中的旧数据。需要保留旧会话、卡片、模型配置或密钥时，请在安装前备份。安装包尚未商业签名，SmartScreen 可能提示“未知发布者”。详见 [Windows 安装说明](./docs/windows-installer.md)。
