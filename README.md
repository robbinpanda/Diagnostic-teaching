...
答疑页不再显示学习卡片栏按钮，也不再挂载右侧文件管理器。知识卡片导出迁入中央“知识卡片库”，继续复用原目录树浏览、文件夹批选、逐张选择和单/双/三列 PDF 排版弹窗，但只接收已归档 `knowledge_card`；“错题卡片库”继续使用现有跨试卷多选与错题集打印样式。导出会打开系统打印面板，选择“另存为 PDF”即可保留 KaTeX 公式与彩色版式。
