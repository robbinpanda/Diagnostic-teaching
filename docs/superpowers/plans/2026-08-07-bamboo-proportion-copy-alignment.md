...
Wrap the three `.bamboo-instance` groups in `<g class="bamboo-position-group" transform="translate(0 192)">`. Assert that the wrapper exists exactly once in both the fragment and standalone preview, while the three instance transforms remain unchanged and the bamboo composition baseline matches the green book cover.
