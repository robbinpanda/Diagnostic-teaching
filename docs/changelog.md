...
- 修复尚未创建 session 时收起左侧会话栏后没有恢复入口的问题；欢迎页现在会显示独立的“展开会话栏”按钮，并用视觉合同测试固定该分支。
...
- 清理旧式 typing/datetime 用法、冗余分支与上下文管理器、异常链缺失、正则歧义和 SQLite 重试返回路径等现有命中。
- 仅对中文全角标点、测试中的有意轮询以及必须保留异常前部分结果的逐项消费做定向豁免，避免静态建议改变运行语义。

### 代码质量专项：可复现的 Python 核心依赖

- 将后端核心运行时的直接依赖范围移入 `requirements-core.in`，生成的 `requirements-core.txt` 固定全部传递依赖并记录分发文件 SHA-256。
- 新增固定版本的 `pip-tools` 工具清单和 Windows 锁定脚本；Docker 与轻量本地运行继续共用同一份核心锁文件。
- 开发测试工具改用精确版本，并显式安装与当前 Starlette 测试客户端匹配的 `httpx2`，避免环境漂移和兼容性警告。
...
- 图片格式校验也集中到同一入口，继续只接受 PNG、JPEG 与 WebP。
- 新增边界值、超限短路、格式拒绝和有效文件读取测试。

### 代码质量专项：Provider 到 SSE 背压

- Provider 流与 SSE 消费者之间改用容量为 32 的有界队列，慢客户端不再导致增量事件无限堆积。
- 终止检测不再依赖向可能已满的队列强塞哨兵；取消慢消费者时可直接取消 Provider 泵，避免满队列死锁。
- 新增快速 Provider 背压、慢消费者关闭、异常前缓冲排空和非法容量测试。

### 代码质量专项：诊断日志异步写入

- 聊天流中的 JSONL/Markdown 诊断日志改由单一后台写入器通过工作线程落盘，不再阻塞 FastAPI 事件循环。
- 后台写入队列设为有界 FIFO；队列满时对日志生产者施加背压，并在应用关闭时排空待写记录。
- 新增事件循环响应性、队列容量、写入顺序和失败隔离测试。

### 代码质量专项：异步聊天与 SQLite 隔离

- `/api/chat/stream` 的同步 SQLite 仓储操作统一通过工作线程执行，数据库 busy timeout 与有限重试不再阻塞 FastAPI 事件循环。
- 新增事件循环响应性测试，验证阻塞型仓储工作执行期间其他异步任务仍可运行。

### 代码质量专项：Docker 默认网络边界

- `compose.local.yml` 的宿主机端口固定绑定到 `127.0.0.1:3000`，避免无认证 API 默认暴露到局域网。
- 运行文档明确 Docker 仅用于本机隔离环境；在认证、TLS 与来源防护完成前不支持远程访问。
- Docker 打包合同测试固定回环地址映射，防止后续配置回退为全网卡暴露。

### 文档入口与侧栏持久化可靠性

- README 收敛为产品定位、快速开始和核心不变量；新增 `docs/README.md` 与 `docs/system-overview.md`，集中说明当前组件职责、输入接纳、run、事件、卡片和恢复主流程，并明确区分现行合同与历史规划资料。
- 本地生成的 `.tmp/` 与 `artifacts/` 统一忽略，不再污染 Git 工作区。
- 桌面侧栏宽度读取/写入在浏览器存储受限时安全降级；改用 `ResizeObserver` 跟随实际 shell 尺寸，并在窗口暂时缩窄后保留用户首选宽度。

### 桌面侧栏宽度调节

- 桌面端左侧导航与主内容之间新增可拖动分隔条，支持鼠标、触控笔及键盘方向键调整，并在本机记住上次宽度。
- 左侧导航最窄保持 220px，主内容最窄保持 640px；中等宽度与移动端继续使用原有抽屉布局，不显示拖动分隔条。

### 三个卡片库的多选删除

- 知识卡片库、错题卡片库和错题集均在多选按钮旁新增删除入口；选中内容后显示数量，删除前统一二次确认。
- 新增卡片与错题集的原子批量删除接口：选中项失效时整批回滚；删除卡片不影响来源会话，删除错题集不影响来源题目卡片。
- 错题集封面固定只显示前三个题目标题，每个标题严格单行，超出宽度时省略显示。

### 卡片库预览压缩

- 知识卡片库与错题卡片库统一使用更紧凑的试卷卡片高度，减少三个预览标题下方的空白。
- 错题卡片库固定只预览前三个题目标题，每个标题严格保持单行，超出宽度时省略显示。

### 卡片导出内容统一与练习模式

- 知识卡片库和错题卡片库的打印内容分别对齐答疑中的知识卡片与题目卡片字段，统一为更紧凑的 A4 纵向固定双列；移除知识导出弹窗的单/双/三列选择。
- 错题集快照新增完整题目卡片 JSON，来源 session 或卡片删除后仍能恢复解题思路、关键步骤、思考来源、易错提醒和最终答案；新增“只看题目摘要”练习模式，用空白作答区替换其他解析内容。
- 错题卡片库改为完全由已归档题目卡片驱动：没有题目卡片的会话不显示，删除会话后已入库卡片继续永久保留；点击条目直接打开题目卡片，删除操作只删除卡片而不删除会话。
- 错题集创建接口改为接收有序 `card_ids`，来源会话已删除的卡片仍可导出和保存快照。
- 新增 Alembic `0015_mistake_set_problem_cards`，并补充数据库迁移、快照恢复、固定双列和答案遮挡回归测试。

### 卡片库入口与导出职责调整

- 删除答疑页顶部的学习卡片栏按钮及右侧文件管理器，桌面和移动端工作台同步移除第三列/右侧抽屉状态；当前 session 的已归档卡片仍可从卡片架打开。
- 左侧导航改为平级的“知识卡片库”“错题卡片库”“错题集”，取消“错题库”父级和二级菜单；原“错题合集”页面统一更名为“错题卡片库”。
- 知识卡片库新增与错题卡片库一致的库内多选入口，支持按整卷或逐张跨卷选择；选完后再打开导出确认弹窗，并只导出已归档知识卡片。

### 历史会话菜单精简

- 左侧历史会话按 Codex 风格保持单行展示完整标题数据，仅在可用宽度不足时由界面显示省略号；完整题目仍保留在悬停提示和搜索数据中。
- 移除每个 session 下方的消息数与更新时间，生成状态改为行内旋转图标，并压缩条目与删除按钮高度，让列表更紧凑整洁。

### 左侧导航悬停横向滚动条修复

- 左侧导航保留原有菜单 hover 位移、颜色和尺寸，同时隐藏横向溢出，避免鼠标移入“错题卡片库”等菜单时底部横向滚动条闪现。
- 新增视觉合同测试，锁定导航只能纵向滚动且 hover 动效继续保留。

### 卡片清理与生成生命周期竞态修复

- “清空全部卡片”不再只依赖进程内的活动 HTTP 流；后端会在同一个 `BEGIN IMMEDIATE` 事务内复查权威 `session_runs`，任一 run 仍为 `queued/running` 时返回 409 且保留全部卡片。
- 清理事务复用 SQLite busy retry，避免浏览器断流后 provider 仍在运行时误删待归档卡片，也避免检查通过到实际删除之间插入新的 run。
- 新增 queued/running 两种无活动浏览器流场景的回归测试；前端布局、样式和展示逻辑不变。

### 输入区与试卷清理修复

- 首页输入区暂时移除初中/高中选择器，底层 `grade_band` 合同与默认 `junior` 保留，供后续其他页面重新接入。
- 推理强度触发器按内容保留最小空间，“推理 · 关闭”等中文档位不再被省略号或下拉箭头遮挡。
- “清空全部会话”现在在同一 SQLite 事务中同步删除全部试卷；前端同时清空试卷状态并使删除前的在途列表请求失效，避免旧试卷重新出现在题图框选页。活动生成流存在时仍整体拒绝清理。

### 错题合集、试卷归档与卡片类型库

- 左侧“历史搜题”改为不可跳转的树标题，仅保留搜索与“试卷 → 题目”快速树；“错题库”父级下设“错题合集”和“错题集”。
- 错题合集支持跨试卷多选，保存或打印时通过 `POST /api/mistake-sets` 写入独立 SQLite 快照；错题集总览展示已保存/打印集合，进入后直接显示 A4 PDF/打印预览。新增 Alembic `0014_mistake_sets`、`mistake_sets` 和 `mistake_set_items`，来源 session 删除后题目文字、题图与来源试卷仍可恢复。
- 知识库中央页面按试卷展示已收纳 `knowledge_card`，进入试卷后展示知识点。待归档与已归档的知识/题目卡都可重新选择试卷位置，或在卡片中创建新的试卷受管目录。
...
- 当前 pending 卡以及从卡片架/卡片库打开的归档卡共用唯一浮动窗口；视口宽度 `>900px` 时整张卡片的非交互区域支持 Pointer Events 拖动，同时保留键盘移动、Home 复位和消息视口边界夹取；删除独立拖动按钮并恢复原灰色圆点，`≤900px` 使用无拖动的全宽安全布局。
