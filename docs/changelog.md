...
- 压缩检查点卡片的上下内边距、题干间距、五个选项高度与底部提交区；自定义回答输入框同步改为两行紧凑高度，让检查点出现时仍能看到更多上下文，同时保留长题干和选项的自然换行。
...
- 知识卡片库新增导出入口，复用原目录树选择和单/双/三列 PDF 弹窗，并只导出已归档知识卡片；错题卡片库保留原有跨试卷多选与错题集打印流程。
