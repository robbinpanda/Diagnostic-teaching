...
- 根据最近 session 的原始日志修复学生说“我完全不会做这道题”后模型直接输出整题 `SUMMARIZE` 的严重流程错误：明确区分“思路信息已收齐”和“教学目标已完成”，最新学生消息表达不会、没思路、不理解或无法开始时，提示词强制先降低台阶讲解。
- 在 system prompt、action 选择提示和 `SUMMARIZE` 边界中重复强调上述约束；学生语义仍由模型结合完整对话判断，后端不增加中文关键词正则或额外的 action 拒绝机制，避免误判正常表达。
...
- 学生发送新问题会先调用显式 interrupt，服务端把当前已经展示的文本保存为 `INTERRUPTED_EXPLANATION` assistant message，再接纳学生原文并启动新一轮生成；刷新后部分讲解与学生问题都不会丢失。
- 普通“停止生成”仍丢弃未完成片段，不会误当成学生提问打断。
- 被打断后的学生原文建立为最高优先级支线；支线解决前禁止 `SUMMARIZE` 和接续原流程。模型明确标记支线解决后自动从原片段断点继续，页面同时保留“回到原讲解”按钮供学生提前手动返回。
- 支线生成期间输入框锁定，第一版只允许一层打断；支线中的普通回复和 checkpoint 仍进入完整历史，返回原讲解后模型可以使用这些内容。
- 本地演示模型识别支线回答和断点续写控制消息，并输出对应完成标记，不再把内部控制 JSON 当成普通学生提问。
- assistant action 与 `detour_active → resuming → resolved` 状态转换改为同一事务提交；返回阶段只有收到 `interruption_resume_completed=true` 才能结束支线。

### 学习卡片不再锁住学生提问

- knowledge/problem card 出现后输入框继续可用；学生发送新问题时，消息接纳与卡片 `deferred_at`、`card.deferred` event 在同一事务保存，卡片自动折叠但仍可展开、编辑、保存或舍弃。
- 待处理卡片与 checkpoint 可同时显示，不再争用单一前端交互槽；刷新或重新打开会话后，SQLite 会恢复卡片的待处理状态。
- 同一 session 最多保留一张未归档卡片；待处理期间 prompt 和后端最终归一化双重禁止生成新卡。稍后处理旧卡不会再额外启动一轮 AI 续讲。
- 卡片抑制现在按整次 run 固定：即使学生在一轮多步骤回答尚未结束时保存旧卡，该 run 的后续步骤仍不能立即生成替代卡，避免出现视觉上像“旧卡仍然存在”的连续重复卡。
- 已经 deferred 的卡片不会被后续学生消息再次延期；API、SQLite 和 durable event 始终保留同一次转换及同一时间戳。
- pending 卡片存在时，即使模型违规生成的新卡片 JSON 本身损坏，后端也会保留可见讲解并安全丢弃新卡，不再让整个 run 失败。
