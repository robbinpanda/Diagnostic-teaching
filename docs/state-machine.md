...
生成 action、assistant message 和待归档 card 在一个 SQLite 事务中写入。新卡片最初 `saved_at=null`，不会出现在右侧卡片库。知识卡片随消息时间线内嵌展示，用户可删除或修改卡片字段和列表项：点击“保存到卡片库”时，前端通过 `CARD_DISMISSED_CONTINUE` 提交最终 `content`，后端在同一事务更新 `content_json/title/saved_at` 与控制命令；首次点击“舍弃”只把按钮切换成“确认舍弃”，第二次点击才提交 `save_to_library=false`，后端在同一事务写控制命令、追加 `card.discarded` 并删除待归档卡片。两条路径之后前端都调用 `/api/chat/stream` 继续。Problem card 仍只归档、不继续。未解决的待归档卡片存在时，`/api/chat/stream` 返回 409，避免绕过确认继续生成。

全局卡片库中的查看不属于阻塞教学工作流。点击右侧卡片时，前端打开靠屏幕右侧且没有暗色 backdrop 的浮层，浮层外保持可交互，以便同时核对对话。已归档 knowledge card 可复用同一编辑器并通过 `PUT /api/cards/{id}` 更新 `title/content_json`；待归档卡片不能走该接口，problem card 也保持只读。

Checkpoint 同样嵌入消息时间线。点击选项只改变浏览器内的待提交选择，只有再点击“提交答案”才调用 answer 接口，避免误触直接落库；后端一次性答案和幂等约束不变。
