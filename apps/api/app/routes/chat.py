...
class SessionStreamCoordinator:
    ...
    async def request_interrupt(self, session_id: str) -> list[SessionRunHandle]:
        """Flag all queued/running work; callers persist status before cancelling tasks."""
        async with self._guard:
            targets = [
                handle
    ...
    async def status(self, session_id: str) -> dict[str, Any]:
        async with self._guard:
            ...
            return {
                ...
                "run_id": running.run_id if running is not None else (handles[0].run_id if handles else None),
                "queued_run_ids": [handle.run_id for handle in handles if handle.state == "queued"],
            }

    async def has_active_streams(self) -> bool:
        async with self._guard:
            # Terminal status may already be durable while provider/ASGI cleanup is
            # still draining. Destructive operations must wait for handle removal.
...
async def chat_stream(payload: ChatStreamRequest, request: Request) -> StreamingResponse:
    ...
    async def event_stream():
        ...
        try:
            ...
            # A queued request must not expose its student input to the preceding run.
            # Persist it only after this run owns the per-session execution lock.
            if request.app.state.sessions.latest_pending_card(payload.session_id) is not None:
                raise RuntimeError("请先关闭并保存当前学习卡片，再继续答疑")
            ...
            while True:
                ...
                async for kind, value in coordinated_generation(coordinator, handle, generation):
                    if kind == "message_delta":
                        # 可见增量还不是业务 action；只有完整 turn 通过事务门闩后才入库。
            ...
            # An ASGI/client disconnect cancels this response task. It is deliberately
            # not classified as an explicit interrupt, even though provider cleanup is identical.
            error = run_error(
                "client_disconnected",
            ...
            terminal = True
            raise
        except Exception as exc:  # surfaced to UI as a structured failed run
            message = str(exc).strip() or exc.__class__.__name__ or "答疑请求失败，请重试"
            code = "provider_error" if isinstance(exc, LlmProviderError) else "run_failed"
            error = run_error(
                code,
                message,
                error_type=exc.__class__.__name__,
