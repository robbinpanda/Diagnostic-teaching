...
一级导航顺序固定：开始答疑 → 历史搜题 → 知识库 → 错题库 → 本地安全存储提示。当前基线的“最近答疑”使用平铺会话列表：搜索只匹配会话标题；每行显示标题、消息数、日期与运行状态，并支持恢复会话和单条删除。忙碌、空列表与无匹配结果状态均须保留。

“清空全部会话”是 `sidebarSectionHeader` 右侧具有可访问名称的图标按钮；清空处理中、存在生成中会话或历史为空时禁用。当前提交不引入试卷分组、试卷级操作或新的历史数据结构。
